...
def main_parser(parser=None):
    ...
    p_generate.add_argument("-s", "--strip_internal_attrs", dest="strip_internal_attrs",
        action='store_true',
        default=False,
        help="remove traceback comments and attributes from generated code (only applicable for verilog backend)")
...
def main_runner(parser, args, design, platform=None, name="top", ports=()):
    if args.action == "generate":
        ...
        if generate_type == "v":
            output = verilog.convert(fragment, name=name, ports=ports, strip_internal_attrs=args.strip_internal_attrs)
